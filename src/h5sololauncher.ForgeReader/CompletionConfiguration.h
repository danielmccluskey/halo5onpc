...
inline void parseConfiguration(const std::vector<unsigned char>& data){
    ...
    auto count=wire.number<uint32_t>();require(count==3 || count==5 || count==6 || count==8 || count==19,"The supported campaign successor set is incomplete.");std::set<uint32_t> seen;
